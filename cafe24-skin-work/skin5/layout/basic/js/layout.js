...
function fixedHeader() { // 210804 서정환 수정
    ...
    let headerL = document.querySelector("header_L");
    let headerR = document.querySelector("header_R");
    let fixed_margin = document.getElementById("contents");
	...
	if(bottomNav){ // 모바일 일체형일 경우
		let logo = document.querySelector("#header .inner .top_nav_box");
    	let logoHeight = logo ? logo.offsetHeight : 0;

		if (scrollY > TopBannerHeight + logoHeight) {
			header.classList.add("fixed");
			header.style.top = '-' + logoHeight + 'px';
			fixed_margin.style.marginTop = header_height + 'px';
		} else {
		...
		if (scrollY > TopBannerHeight) {
			if (!(!(headerL || headerR) && window.innerWidth >= 1024)) { // pc 좌우형일때는 작동하지 않음
				header.classList.add("fixed");
				fixed_margin.style.marginTop = header_height + 'px';
			}
...
function handleNav() {
    ...
    var btnCloses = document.querySelectorAll('#aside .btnClose, #aside + .btnClose');
    var dimmed = document.querySelector('#layoutDimmed');
    btnNavs.forEach( function(btnNav) {
        btnNav.addEventListener('click', function(){
            document.body.classList.toggle('expand');
			if(document.body.classList.contains('searchExpand')){
...
function top_category() {
	/* 상단카테고리 */
	jQuery('#header .top_category li').each(function() {
		var $li = jQuery(this);
		var timeoutId;

		$li.on('mouseenter', function() {
			clearTimeout(timeoutId); // 기존 timeout 제거
			$li.addClass('on');
            $li.siblings('li').removeClass('on');
		});

		$li.on('mouseleave', function() {
            if ($li.is('.sub_cate01 > li')) {
                timeoutId = setTimeout(function() {
                    $li.removeClass('on');
                }, 300);
            } else if ($li.is('.sub_cate02 > li')) {
                timeoutId = setTimeout(function() {
                    $li.removeClass('on');
                }, 300);
            } else {
...
function ifmore(){
	/* 상품 썸네일 관심상품 출력 & 숨김 */
	setTimeout(function(){
		jQuery('.ec-base-product .prdList .wish').each(function(){
			var isstatus = jQuery(this).children('img').attr('icon_status');
			if ( isstatus == 'on' ) {
				jQuery(this).addClass('on');
			}
		});
		jQuery('.ec-base-product .prdList .wish').click(function(){
			var $img = jQuery(this).children('img');
        	var isstatus = $img.attr('icon_status');
